...
app.post('/register', async function (req, res) {
    const data = hybrid_inst.decrypt(req.body);
    ...
    if (existing_row) {
        ...
        await db.run('INSERT INTO main(username, password, notes, note_salt) VALUES (?, ?, ?, ?)', [username, await argon2.hash(password), null, argon2.salt()]);
        res.json({ 'success': true });
    }
});
async function authenticate(username, password, hash) {
    if (await argon2.verify(hash, password)) {
        var rehash = await argon2.rehash(hash, password);
        if (rehash)
            await db.run('UPDATE main SET password = ? WHERE username = ?', [rehash, username]);
        return true;
    }
    else {
        return false;
...
async function pwdToAes(password, salt) {
    return await argon2.hash(password, salt, true, 32);
}
app.post('/login', async function (req, res) {
    const data = hybrid_inst.decrypt(req.body);
    const [username, password, pubkey] = [data.username.trim(), data.password.trim(), data.public_key.trim()];
    const row = await db.get('SELECT password, notes, note_salt FROM main WHERE username = ?', username);
    if (row) {
...
app.post('/save', async function (req, res) {
    ...
    const row = await db.get('SELECT password, note_salt FROM main WHERE username = ?', username);
    if (row) {
        if (await authenticate(username, password, row.password)) {
            var aes_key = await pwdToAes(password, row.note_salt);
            var notes_encrypted = AES.encrypt(JSON.stringify(notes), aes_key);
            await db.run('UPDATE main SET notes = ? WHERE username = ?', [notes_encrypted, username]);
            res.json({ 'success': true });
        }
        else {
